...
Write-Host "VERIFICATION FEATURE 011 : MODIFICATION DES OBJETS DANS LE MODULE CONFIG" -ForegroundColor Cyan
Write-Host "===================================================================" -ForegroundColor Cyan
...
Write-Host "Tests Unitaires et Integration Reussis (0 warning, 0 error) !" -ForegroundColor Green

# 2. Test d integration des endpoints REST PUT (si serveur actif)
Write-Host "`n[2/3] Test d integration des endpoints REST PUT..." -ForegroundColor Yellow

try {
    # 2.1 Recuperer le catalogue existant
    $products = Invoke-RestMethod -Uri "http://localhost:5000/api/catalog/products" -Method Get -TimeoutSec 2
    ...
    $staffList = Invoke-RestMethod -Uri "http://localhost:5000/api/staff" -Method Get -TimeoutSec 2
    ...
    $printers = Invoke-RestMethod -Uri "http://localhost:5000/api/printers" -Method Get -TimeoutSec 2
    ...
    Write-Host "Note: Serveur API local non demarre sur le port 5000 (les tests unitaires et d integration en memoire valident les endpoints)" -ForegroundColor Gray
}
